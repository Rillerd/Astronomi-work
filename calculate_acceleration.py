"""
position  = np.zeros((n_dim, n_particle))
...
r = np.array([0.1,1,2,3,4,5,6,7])  # get r position of particle
theta = np.array([0.2, 0.3, 0.6, 0.8, 0.9, 1, 1.1, 1.2])  # get theta for each particle
...
velocity = np.sqrt(G * Galaxy_mass / r) / denumerator


#vrot= np.sqrt(G * Galaxy_mass / r)
#velocity[0, :] = velocity[0, :] / denumerator * vrot
#velocity[1, :] = velocity[1, :] / denumerator * vrot # this is just the direction of the velocity, not the acutal velocity # this is the value of the velocity with the velocity vectors. Dont know how to deal with negative though
print(velocity)

   velocity[0] = -position[1] / r  # transpose the position vector and take negative of one to get an orthogonal direction to the position vector, perpendicular direction
    ...
    for i in range(n_particle):
        ...
        vrot = np.sqrt(G * mass_r / r[i])
        velocity[0, i] = velocity[0, i] * vrot
        velocity[1, i] = velocity[1, i] * vrot # this is just the direction of the velocity, not the acutal velocity
...
def mkGalaxy2D(n_particle, Galaxy_radius, Galaxy_mass, x0=0.0, y0=0.0, vx0=0.0, vy0=0.0):
    ...
    velocity[0] = -position[1] / r  # transpose the position vector and take negative of one to get an orthogonal direction to the position vector, perpendicular direction
    velocity[1] = position[0] / r  # direction of velocity vector
    ...
    for i in range(n_particle):
        ...
        vrot = np.sqrt(G * mass_r / r[i])  # magnitude of rotational velocity

        velocity[0, i] = velocity[0, i] * vrot
        velocity[1, i] = velocity[1, i] * vrot 
...
#multiplied the vrot with 0.1 because I have all mass in the center for each particle and not the mass which is within a radius and thus the ones in the middle have too large vrot
...
#multipled 0.1 with galaxy radius because the particles may be too heavy and thus just shoot out from the galaxy
